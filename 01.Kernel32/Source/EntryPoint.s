...
PROTECTEDMODE:
	...
	mov esp, 0xFFFE		; ESP 레지스터의 주소를 0xFFFE로 설정
	mov ebp, 0xFFFE		; EBP 레지스터의 주소를 0xFFFE로 설정
	...
	jmp dword 0x18: 0x10200		; CS 세그먼트 셀렉터를 커널 코드 디스크립터(0x18)로 변경하면서
								; 0x10200 주소(C언어 커널이 있는 주소)로 이동.
								; C언어 커널이 존재하는 0x10200 주소로 이동하여 C 언어 커널 수행
...
PRINTMESSAGE:
	...
	mov ebp, esp	; 베이스 포인터 레지스터(BP)를 스택 포인터 레지스터(SP)의 값을 설정
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; Y 좌표를 이용하여 먼저 라인 주소를 구함
	mov eax, dword [ ebp + 12 ]	; 파라미터 2(화면 좌표 Y)를 EAX 레지스터에 설정
	mov esi, 160				; 한 라인의 바이트 수(2 * 80 컬럼)를 ESI 레지스터에 설정
	mul esi						; EAX 레지스터와 ESI 레지스터를 곱하여 화면 Y 주소 계산
	mov edi, eax				; 계산된 화면 Y 주소를 EDI 레지스터에 설정

	; X 좌표를 이용해서 2를 곱한 후 최종 주소를 구함
	mov eax, dword [ ebp + 8 ]	; 파라미터 1(화면 좌표 X)를 EAX 레지스터에 설정
	mov esi, 2					; 한 문자를 나타내는 바이트 수(2)를 ESI 레지스터에 설정
	mul esi						; EAX 레지스터와 ESI 레지스터를 곱하여 화면 X 주소를 계산
	add edi, eax				; 화면 Y 주소와 계산된 X 주소를 더해서
								; 실제 비디오 메모리 주소를 계산

	; 출력할 문자열의 주소
	mov esi, dword [ ebp + 16 ]	; 파라미터 3(출력할 문자열의 주소)

.MESSAGELOOP:	; 메시지를 출력하는 루프
	mov cl, byte [ esi ]	; ESI 레지스터가가리키는 문자열 위치에서 한 문자를
							; CL 레지스터에 복사
							; CL 레지스터는 ECX 레지스터의 하위 1byte를 의미
							; 문자열은 1byte면 충분하므로 ECX 레지스터의 하위 1byte만 사용합니다.

	cmp cl, 0		; 복사된 문자와 0을 비교
	je .MESSAGEEND	; 복사한 문자의 값이 0이면 문자열이 종료되었음을 의미하므로
					; .MESSAGEEND로 이동하여 문자 출력 종료

	mov byte [ edi + 0xB8000 ], cl	; 0이 아니라면 비디오 메모리 주소 0xB8000 + EDI에 문자를 출력
