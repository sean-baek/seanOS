...
TOTALSECTORCOUNT: dw 0x02	; 부트로더를 제외한 FS64 OS 이미지의 크기 최대 1152 섹터(0x90000byte)까지 가능
...
.SCREENCLEARLOOP:	; 화면을 지우는 루프
	...
	mov byte [ es: si + 1 ], 0x0A	; 비디오 메모리의 속석이 위치하는 주소에 0x0A(검은 바탕에 밝은 녹색)을 복사
	...
	jl .SCREENCLEARLOOP	; SI 레지스터가 80 * 24 * 2보다 작다면
				; 아직 지우지 못한 영역이 있으므로 .SCREENCLEARLOOP 레이블로 이동
...
READDATA:	; 디스크를 읽는 코드의 시작
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	add si, 0x0020	; 512(0x200)byte만큼 읽었으므로, 이를 세그먼트 레지스터 값으로 변환
	...
	mov byte [ SECTORNUMBER ], al	; 증가시킨 섹터 번호를 SECTORUNMBER에 다시 설정
	...
	; 만약 헤드가 1 -> 0로 바뀌었으면 양쪽 헤드를 모두 읽은 것이므로 아래로 이동하여 트랙 번호를 1 증가
...
PRINTMESSAGE:
	...
	add di, ax		; 화면 Y 주소와 계산된 X 주소를 더해서
				; 실제 비디오 메모리 주소를 계산
...
;;;;;;;;;;;;;
; 부트 로더 시작 메시지
MESSAGE1 : db 'FS64 OS   Boot Loader Start~!!', 0	; 출력할 메시지 정의
