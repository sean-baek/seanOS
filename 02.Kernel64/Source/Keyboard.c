...
BOOL kActivateKeyboard(void)
{
	...
	// 입력 버퍼(포트 0x60)로 키보드 활성화(0xF4) 커맨드 전달하여 키보드로 전송
...
{
	// 출력 버퍼(포트 0x60)에 데이터가 읽을 때까지 대기
...
BOOL kChangeKeyboardLED( BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn )
{
	...
	for(j = 0; j < 100; j++)
	{
		for(i = 0; i < 0xFFFF; i++)
		{
			// 출력 버퍼(포트 0x60)가 있으면 데이터를 읽을 수 있음
			if(kIsOutputBufferFull() == TRUE)
			{
				break;
			}
